...
{   int i;
    ...
    if (atof(str)<0.0 || atof(str)>100.0)  //判断是否符合程序逻辑 
	{
		return -1;
	}
	else
	{
		return atof(str);   //转化为double型 
	}
}
...
{   if(SearchItem(n_ID,p_head)==NULL)
    ...
    Stu *p_temp1=SearchItem(n_ID-1,p_head);   //结点前后关联 
...
int Sort(Stu *p_head,int n_ID,int n_subject)   //院系总排名，传入头指针、学生编号、所要排名的数据类型 
{
	...
	int i = 1;
	int sum = 1;
    if(n_subject == 1)   //用SearchItem()函数对每一个结点该编号数据值进行检索，遇到比被排序结点数值大时sum+1
    {
...
{   
	...
	Stu *p_temp1 = SearchItem(n,p_head);    //搜索新结点插入位置的前后结点 
    if(p_temp1==NULL)
    {
        printf("提示：未找到该编号");
		return;	
